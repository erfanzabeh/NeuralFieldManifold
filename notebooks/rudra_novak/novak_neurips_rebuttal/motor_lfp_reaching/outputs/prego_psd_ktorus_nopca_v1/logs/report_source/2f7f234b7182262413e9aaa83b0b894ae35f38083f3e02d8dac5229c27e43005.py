...
def examples(root, audit, embedding, diagnostics):
    ...
    for monkey in ("M", "T"):
        ...
        fig.legend(handles, labels, loc="outside lower center", frameon=False, ncol=2, fontsize=8)
...
def write_report(root, audit, embedding, diagnostics):
    ...
    added = tests[tests.family == "additive"]
    positive = added[(added.p_holm < .05) & (added.mean_difference > 0)]
    lines += ["", f"Added value: {len(positive)} of {len(added)} planned additions show a positive "
              "effect significant at Holm p < 0.05. Do not infer improved decoding from positive mean differences alone."]
    lines += ["", "## Planned comparisons", "", "Mean paired differences with recording-day-clustered 95% CIs. "
              ...
              "CIs are unadjusted and target the recording-weighted mean, whereas tests use paired day summaries; "
              "a CI excluding zero need not correspond to a significant adjusted p-value.", "",
    ...
    lines += ["", "## Geometry checks", "", "Normalized residual is SSE/TSS in the preprocessed delay cloud, "
              "not decoding error. Numerical convergence does not establish a good geometric fit.", ""]
    for monkey in ("M", "T"):
