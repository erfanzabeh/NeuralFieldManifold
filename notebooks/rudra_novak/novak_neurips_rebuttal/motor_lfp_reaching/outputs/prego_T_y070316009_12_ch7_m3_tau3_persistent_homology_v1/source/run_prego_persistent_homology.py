...
def initialize(clouds, trials):
    ...
    verify_protected()
    immutable_json(OUTPUT / 'analysis_provenance.json', {
        'analysis_source': {str(Path(__file__).resolve()): digest(__file__)},
    ...
    for source, destination in [(Path(__file__), OUTPUT / 'source' / Path(__file__).name),
                                (SOURCE / 'inputs/selected_trials.csv', OUTPUT / 'inputs/selected_trials.csv'),
...
def verify():
    ...
    for name, table in expected.items():
        saved = pd.read_csv(OUTPUT / 'tables' / f'{name}.csv', keep_default_na=False)
        reference = table.fillna('')
    ...
    for section in ('analysis_source', 'inputs'):
        for path, value in provenance[section].items():
