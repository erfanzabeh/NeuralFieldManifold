...
def environment_info():
    return dict(python=sys.version, executable=sys.executable, platform=platform.platform(),
                packages={name: importlib.metadata.version(name) for name in
                          ["numpy", "scipy", "scikit-learn", "pandas", "matplotlib", "joblib", "h5py"]})


def fold_binding(embedding, fold):
    payload = dict(embedding=embedding, fold=int(fold))
    return hashlib.sha256(json.dumps(clean_json(payload), sort_keys=True).encode()).hexdigest()


def validate_fold_checkpoint(saved, embedding, fold, raw_trial_indices):
    if "binding" not in saved or str(saved["binding"]) != fold_binding(embedding, fold):
        raise ValueError("Checkpoint belongs to a different fold or embedding")
    if not np.array_equal(saved["raw_trial_indices"], raw_trial_indices):
        raise ValueError("Checkpoint trial order changed")
    if not np.array_equal(saved["feature_names"], embedding["feature_names"]):
        raise ValueError("Checkpoint feature schema changed")
    if saved["features"].shape != (len(raw_trial_indices), len(embedding["feature_names"])):
        raise ValueError("Checkpoint feature matrix shape changed")


def artifact_hashes(folder):
    return {p.name: sha256(p) for p in sorted(Path(folder).iterdir())
            if p.is_file() and p.suffix in (".json", ".csv", ".npz") and not p.stem.endswith("_complete")}


def verify_artifacts(folder, expected):
    if not expected:
        raise ValueError("No checkpoint integrity manifest")
    for name, digest in expected.items():
        path = Path(folder) / name
        if not path.is_file() or sha256(path) != digest:
            raise ValueError(f"Missing or changed checkpoint artifact: {name}")
...
def initialize_run(root, config, provenance, resume=False):
    ...
    if root.exists():
        ...
        if manifest["environment"] != environment_info():
            raise ValueError("Environment changed; refusing mixed-environment checkpoints")
        return manifest
    if resume:
...
def verify_selection(current, previous):
    for key in ("raw_trial_indices", "folds", "labels", "original_trial_number"):
...
def fit_recording(path, root, config):
    ...
    if done.exists():
        ...
        if meta["status"] == "eligible":
            verify_artifacts(folder, meta.get("artifacts"))
        return meta
    ...
    selection_hash = hashlib.sha256(raw_indices.tobytes()+folds.tobytes()+y.tobytes()+current["original_trial_number"].tobytes()).hexdigest()
    ...
    for fold in range(5):
        ...
        if feature_path.exists():
            with np.load(feature_path) as saved:
                validate_fold_checkpoint(saved, embedding, fold, raw_indices)
            if (folder / f"fold_{fold}_diagnostics.csv").exists():
                continue
        ...
        save_arrays(feature_path, features=features, selection_hash=selection_hash,
                    binding=fold_binding(embedding, fold),
    ...
    meta["artifacts"] = artifact_hashes(folder)
    write_json(done, meta)
    return meta
...
def decode_recording(path, root, config):
    ...
    if done.exists():
        ...
        if completed["status"] == "eligible":
            verify_artifacts(folder, completed.get("artifacts"))
        return completed
    if meta["status"] != "eligible":
        write_json(done, meta)
        return meta
    ...
    if usable:
        ...
        completed = len(null_rows)//len(NULL_METHODS)
        for permutation in range(completed, config["null_permutations"]):
            ...
            null = decode(d["power"], d["frequency"], d["bands"], geometry, labels, d["folds"], NULL_METHODS)
...
def main():
    ...
    config = dict(schema_version=2, seed=42, window_ms=[-500, 0], delay="short", directions=6,
